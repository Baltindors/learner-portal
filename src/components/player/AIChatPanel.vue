<template>
  <div
    :class="['flex flex-col h-full', preferredMode === 'audio' ? 'text-white' : 'text-slate-900']"
  >
    <div class="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar">
      ...
      >
        <span class="text-[10px] font-bold uppercase tracking-widest mb-1 opacity-50">{{
          msg.sender
        }}</span>
      ...
      <div
        v-if="isListening"
        class="flex flex-col items-center justify-center py-6 text-indigo-500"
      >
        <div class="flex items-center gap-1 mb-2">
          <div class="w-1 h-3 bg-indigo-500 rounded-full animate-pulse"></div>
          <div
            class="w-1 h-5 bg-indigo-500 rounded-full animate-pulse"
            style="animation-delay: 0.1s"
          ></div>
          <div
            class="w-1 h-4 bg-indigo-500 rounded-full animate-pulse"
            style="animation-delay: 0.2s"
          ></div>
          <div
            class="w-1 h-2 bg-indigo-500 rounded-full animate-pulse"
    ...
    <div
      ...
      :class="
        preferredMode === 'audio' ? 'border-white/10 bg-slate-900/50' : 'border-slate-200 bg-white'
      "
    >
...
const formatMessage = (text) => {
  ...
  return text.replace(
    /\[Source: (.*?)\]/g,
    `<span class="inline-block mx-1 px-1.5 py-0.5 bg-indigo-100 text-indigo-700 text-[10px] rounded cursor-help border border-indigo-200" title="$1">Citation</span>`,
  )
}
...
const handleAiResponse = (userText) => {
  ...
  if (q.includes('dosage') || q.includes('dose') || q.includes('dosages')) {
    aiText =
      'Based on the latest protocols discussed in this session, the recommended starting dosage is 10mg daily, escalating as tolerated. Adjustments should be made for renal impairment. [Source: NCCN Guidelines 2026] [Source: FDA Prescribing Info]'
  } else {
